...
import { APP_URL, CHANNEL_URL, DEFAULT_DESCRIPTION } from "./metadata";
import { absoluteOriginUrl } from "./request-origin";
import { SITEMAP_INDEX_PATH } from "./sitemap-xml";
...
Sitemap: ${absoluteOriginUrl(origin, SITEMAP_INDEX_PATH)}
`);
}

export function llmsTxt(origin: string) {
  const siteUrl = (path: string) => absoluteOriginUrl(origin, path);
  const topicGuides = BLOG_TOPIC_GROUPS.map(
    ({ name, summary, hub }) => `### ${name}\n\n${summary}\n\n- ${hub.title}: ${siteUrl(hub.sourceUrl)}`,
  ).join("\n\n");
  const body = `# 독스헌트
...
- 서비스 소개 및 시작: ${siteUrl("/")}
- 요금제와 기능 비교: ${siteUrl("/pricing")}
- 2026 지원사업 합격 환급 조건: ${siteUrl("/refund-event")}
- 사업계획서 예시 샘플: ${siteUrl("/sample")}
...
- 회사와 콘텐츠 작성·검수 원칙: ${siteUrl("/about")}
- 사업자등록 및 통신판매업 신고 정보: ${siteUrl("/business_info")}
